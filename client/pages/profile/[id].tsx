...
function PlanCard({ plan, planExpiresAt, isOwnProfile, onUpgrade }: PlanCardProps) {
  ...
  return (
    <div className={`rounded-xl border-2 p-3 sm:p-4 ${PLAN_BORDER[plan]} ${PLAN_BG[plan]} transition-all`}>
      <div className="flex items-start sm:items-center justify-between flex-wrap gap-2 sm:gap-3">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          {cfg.badge && <span className="text-xl sm:text-2xl flex-shrink-0">{cfg.badge}</span>}
          <div className="min-w-0">
            ...
            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              ...
              {isActive && planExpiresAt && !expired && (
                <span className="ml-1 sm:ml-2 text-xs">· Expires {formatExpiry(planExpiresAt)}</span>
              )}
            </p>
          </div>
        </div>
        {isOwnProfile && (plan === 'free' || !!expired) && (
          <button
            onClick={onUpgrade}
            className="flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-white font-semibold text-xs sm:text-sm px-3 sm:px-4 py-1.5 sm:py-2 rounded-full transition shadow-sm flex-shrink-0"
          >
            ⭐ Upgrade Plan
          </button>
        )}
        {isOwnProfile && isActive && !expired && (
          <button
            onClick={onUpgrade}
            className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 hover:underline font-medium flex-shrink-0"
          >
            Change plan
          </button>
        )}
...
const ProfilePage: NextPage = () => {
  ...
  const [dlMeta,        setDlMeta]        = useState<DlMeta>({ isPremium: false, dailyDownloadCount: 0, dailyLimit: 1 });
  ...
  const fetchDownloads = useCallback(async () => {
    ...
    setDlMeta({ isPremium: data.isPremium, dailyDownloadCount: data.dailyDownloadCount, dailyLimit: data.dailyLimit });
  }, [isOwnProfile]);
  ...
  if (!profile) return <div className="text-center mt-20 text-gray-500">User not found.</div>;
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-white">
      ...
      {/* ── Banner ────────────────────────────────────────────────────────── */}
      <div
        className="w-full h-28 sm:h-40 md:h-56 bg-gradient-to-r from-blue-600 to-purple-600 bg-cover bg-center"
      ...
      <div className="max-w-5xl mx-auto px-3 sm:px-4">
        ...
        {/* ── Profile header ────────────────────────────────────────────── */}
        <div className="flex flex-col xs:flex-row xs:items-end gap-3 sm:gap-4 -mt-10 sm:-mt-12 mb-5 sm:mb-6">
          <img
            src={getAvatarUrl(profile.photoURL, profile.displayName)}
            alt={profile.displayName}
            className="w-20 h-20 sm:w-24 sm:h-24 rounded-full border-4 border-white dark:border-gray-950 shadow-lg flex-shrink-0 object-cover"
          />
          <div className="flex-1 pb-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-xl sm:text-2xl font-bold truncate">{profile.displayName}</h1>
              {profile.isPremium && (
                <span className="text-xs bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 font-semibold px-2 py-0.5 rounded-full whitespace-nowrap">
                  ⬇ Download Premium
                </span>
              )}
              {userPlan !== 'free' && (
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 whitespace-nowrap">
            ...
            {profile.description && (
              <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1 max-w-xl line-clamp-2">
                {profile.description}
              </p>
            )}
          </div>
          {isOwnProfile && (
            <div className="pb-1 flex-shrink-0">
              <Link
                href="/settings"
                className="text-xs sm:text-sm px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition whitespace-nowrap"
              >
                Edit profile
              </Link>
            </div>
        ...
        {isOwnProfile && (
          <div className="space-y-3 mb-5 sm:mb-6">
            ...
            <div className={`rounded-xl border-2 p-3 sm:p-4 transition-all ${
              dlMeta.isPremium
              ...
              <div className="flex items-start sm:items-center justify-between flex-wrap gap-2 sm:gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="font-semibold text-sm sm:text-base text-gray-900 dark:text-white">Download Premium</h3>
                    {dlMeta.isPremium
                ...
                {!dlMeta.isPremium && (
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs sm:text-sm px-3 sm:px-4 py-1.5 sm:py-2 rounded-full transition flex-shrink-0"
                  >
                    ⬇ Upgrade — ₹199
                  </button>
                )}
        ...
        {/* ── Tabs ──────────────────────────────────────────────────────── */}
        <div className="flex overflow-x-auto scrollbar-hide border-b border-gray-200 dark:border-gray-800 mb-4 sm:mb-6">
          {(['videos', ...(isOwnProfile ? ['downloads'] : [])] as Tab[]).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-4 sm:px-5 py-2.5 sm:py-3 text-sm font-medium capitalize border-b-2 transition-colors -mb-px whitespace-nowrap ${
                tab === t
        ...
        {/* ── Videos tab ────────────────────────────────────────────────── */}
        {tab === 'videos' && (
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 pb-12">
            {videos.length === 0 && (
            ...
            {videos.map(video => (
              <div key={video._id} className="group">
                ...
                <div className="mt-1.5 sm:mt-2 px-0.5">
                  <Link href={`/video/${video._id}`} className="font-medium text-xs sm:text-sm line-clamp-2 hover:text-blue-600 transition-colors">
                    {video.title}
                  </Link>
          ...
          <div className="pb-12">
            ...
            <div className="space-y-2 sm:space-y-3">
              {downloads.map((entry, i) => {
                ...
                return (
                  <div key={i} className="flex gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-xl bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 transition-colors">
                    <Link href={`/video/${video._id}`} className="flex-shrink-0">
                      <div className="relative w-28 xs:w-36 aspect-video rounded-lg overflow-hidden bg-gray-200 dark:bg-gray-800">
